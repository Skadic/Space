...
public class CRC16 {
    ...
    private static int CRC16BEGIN = 65535;
    ...
    public static int calculate(String s) {
        int crc16 = CRC16BEGIN;
        try {
            for (byte aByte : s.getBytes("UTF-8"))
                crc16 = (aByte & 0xFF) ^ (crc16 & 0xFF) << 8 ^ CRC16_Lookup[(crc16 >> 8)];
    ...
    public static int getCRC(String drugID, String drugName, String drugShort, String infRate, String infUnit) {
        int result = calculate(CRC16BEGIN, drugID);
        result = calculate(result, drugName);
        result = calculate(result, drugShort);
        result = calculate(result, infRate);
        result = calculate(result, infUnit);

        return result;
    }

    public static int getTotalChecksum(List<Integer> checksums) {
        int result = calculate(CRC16BEGIN, checksums.get(0).toString());

        for (int i = 1; i < checksums.size(); i++) {
            result = calculate(result, checksums.get(i).toString());
        }
